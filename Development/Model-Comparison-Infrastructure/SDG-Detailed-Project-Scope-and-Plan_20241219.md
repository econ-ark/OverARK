---
title: SDG Detailed Project Scope and Plan
tags: [ARK, Grants]
...
Central to the deployment of deep learning methods to solve heterogeneous agent models is standardized infrastructure to enable robust comparisons across deep learning and traditional solution methods.

The purpose of this project is to develop infrastructure that will let us systematically compare alternative computational solutions to what is in principle "the same" model
- The marquee comparison will be for the Krusell-Smith model:
    - Specifically, the comparison will be between 
        - the Maliar, Maliar, and Winant (2021) deep learning solution
            - which is already implemented as a REMARK (see below)
        - other solutions that are (also) already available in the econ-ark ecosystem
- The comparison infrastructure should be developed with an eye to generality
    - Generality will be demonstrated by applying the infrastructure to another problem
        - Because of its simplicity, the other problem will be the Aiyagari model

Steps to accomplish the goals must be taken in the order in which they are outlined below
...
- Familiarize yourself with and reproduce error metrics:
  ...
  - For **Aiyagari**:
      ...
      - Determine metrics commonly used to compare a new solution to Aiyagari's

## Step: Development and Implementation
- Implementation and testing of `ModelComparison` Class
- Instance of a `ModelComparison` class will ingest: 
    - economic parameters 
    - solution-specific specifications (grid size etc.) each solution method
- and will produce
    - Outputs for the solution methods identified (and implemented) as above. 
...
1. Accept an input that is a Markdown document
   - Which should be a brief description of the models being compared
3. Accept model primitive parameters:
   ...
   - Anything else that defines the mathematical structure of the problem, e.g.:
       - The serial correlation coefficient for the income process
...
- **`solve(method: str)`**:
  - Solve the model using the specified method:
    - `krusell_smith/HARK`: Fixed-point iteration based on the KS log-linear law of motion.
